...
namespace UniversidadDB.Controllers
{
    ...
    public class AdminMallaController : ControllerBase
    {
        ...
        // Crea o actualiza un item (CarreraId + CursoId)
        ...
        public async Task<IActionResult> UpsertMallaItem(int carreraId, [FromBody] MallaUpsertDto dto)
        {
            ...
            if (dto.Ciclo <= 0) return BadRequest("Ciclo inválido.");
            if (dto.Creditos < 0) return BadRequest("Créditos inválidos.");
            ...
            // Devuelve un objeto limpio (evita problemas de navegación/ciclos)
            return Ok(new
            {
                row.CarreraId,
                row.CursoId,
        ...
        public async Task<IActionResult> AddPrereq(int cursoId, int cursoPrereqId)
        {
            if (cursoId == cursoPrereqId)
                return BadRequest("No puede ser prereq de sí mismo.");
            ...
            if (already) return Ok(new { CursoId = cursoId, CursoPrereqId = cursoPrereqId, Exists = true });
